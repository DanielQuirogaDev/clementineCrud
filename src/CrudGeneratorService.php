...
class CrudGeneratorService 
{
    ...
    public function Generate() 
    {
        ...
        if(!$this->force) { 
            ...
            if(file_exists(base_path().'/resources/views/'.$this->viewFolderName.'/add.blade.php')) { $this->output->info('Add view already exists, use --force to overwrite'); return; }
                   if(file_exists(base_path().'/resources/views/'.$this->viewFolderName.'/edit.blade.php')) { $this->output->info('Edit view already exists, use --force to overwrite'); return; }
    ...
    protected function deletePreviousFiles($tablename, $existing_model) {
        $todelete = [
                ...
                base_path().'/resources/views/'.str_plural($tablename).'/edit.blade.php',
            ];
        if(!$existing_model) {
